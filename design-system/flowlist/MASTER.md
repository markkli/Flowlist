...
**Direction:** Quiet editorial focus utility
...
Each appearance preset defines the whole workspace palette through `frontend/src/features/appearance/palettes.css`. Grove uses forest and olive, Coast blue and slate, Hills warm earth tones, Linen taupe, Sage botanical green, Slate blue-gray, and Clay muted terracotta. Each has light and dark surfaces and accents.

Use semantic tokens (`--accent`, `--on-accent`, `--surface`, `--ink`, `--muted`) for controls and text. Legacy forest/sage/sand tokens alias these roles. Heatmap levels blend the same accent into the surface. Keep destructive red independent of decorative presets. Borders remain translucent and quiet.
...
- Use an atmospheric page cover matching the selected appearance related to the Pomodoro panel, followed by a quiet creation toolbar. Direction headers stay content-sized with title, concise counts, Add, and More.
...
- Preserve explicit parent completion, ancestor reopening, completed archives. Ritual checkout offers every unfinished level; parent selection cascades to subtasks.
...
- Keep “Add to plan” available during focus and breaks in a nested modal that does not pause or dismiss the running timer. It captures one task into the shared Tasks list or an existing active project; creating an entirely new direction stays in the full Plan view.
...
- The end-of-ritual dialog accepts an optional reflection and can create missing work in Tasks, an existing group, or a new project before attribution.
...
- Keep Plan headers compact at narrow widths. Preserve the selected appearance palette and system typography.

## Brand assets

Use `/brand/flowlist-mark.svg` and `/favicon.svg` for an upright serif F on an ink tile, with ivory lettering and a restrained brass frame. Avoid athletic diagonals, neon, and swooshes.

## Appearance

Offer three matched landscapes (Grove, Coast, Hills) and four theme-aware solids (Linen, Sage, Slate, Clay). Use independent Pomodoro and Plan artwork switches, immediate preview, device persistence, and Reset. Keep artwork confined to these two panels. Apply each preset’s coordinated colors throughout the workspace, including navigation, buttons, timer, Plan, history, and heatmap. Turning artwork off keeps the chosen palette. Preserve readable contrast and consistent hierarchy in both modes.
