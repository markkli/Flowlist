...
const ACTIVE_FOCUS_KEY = "flowlist-active-focus";
const celebratedGoals = new Set();
let timerHandle = null;

async function api(path, options = {}) {
  const response = await fetch(`${API_BASE}${path}`, { headers: { "Content-Type": "application/json" }, ...options });
  if (!response.ok) { const error = new Error(`${options.method || "GET"} ${path} failed: ${response.status}`); error.status = response.status; throw error; }
...
async function loadTasks(goalId) {
  ...
  const leaves = leafTasks(tasks); section.querySelector(".goal-progress-copy").textContent = leaves.length ? `${leaves.filter((task) => task.completed).length} of ${leaves.length} steps complete` : "No steps yet";
...
function startFocus(task, restored = null) { clearInterval(timerHandle); const planned = restored?.planned_seconds || task.estimated_minutes * 60; const deadline = restored?.deadline || Date.now() + planned * 1000; let left = Math.max(0, Math.ceil((deadline - Date.now()) / 1000)); let logged = false; localStorage.setItem(ACTIVE_FOCUS_KEY, JSON.stringify({ task_id: task.id, title: task.title, estimated_minutes: task.estimated_minutes, planned_seconds: planned, deadline })); focusPhaseLabel.innerHTML = "<i></i>Focus"; focusTaskTitle.textContent = task.title; focusGoalTitle.textContent = restored ? "Session resumed" : "Current task"; focusTime.innerHTML = `${formatTime(left)}<span>Remaining</span>`; focusStop.textContent = "End session"; overlay.classList.remove("hidden"); const log = (completed) => { if (logged) return; logged = true; clearSavedFocus(); return api(`/tasks/${task.id}/sessions`, { method: "POST", body: JSON.stringify({ actual_minutes: Math.floor((planned - left) / 60), completed }) }).then(loadDashboard); }; focusStop.onclick = () => { clearInterval(timerHandle); overlay.classList.add("hidden"); log(false); }; const tick = () => { left = Math.max(0, Math.ceil((deadline - Date.now()) / 1000)); focusTime.innerHTML = `${formatTime(left)}<span>Remaining</span>`; if (left <= 0) { clearInterval(timerHandle); log(true); startBreak(); } }; if (left <= 0) tick(); else timerHandle = setInterval(tick, 1000); }
function restoreSavedFocus() { try { const saved = JSON.parse(localStorage.getItem(ACTIVE_FOCUS_KEY)); if (!saved?.task_id || !saved?.deadline) return; startFocus({ id: saved.task_id, title: saved.title, estimated_minutes: Math.ceil(saved.planned_seconds / 60) }, saved); } catch (error) { clearSavedFocus(); } }
function startBreak() { let left = BREAK_SECONDS; focusPhaseLabel.innerHTML = "<i></i>Break"; focusTaskTitle.textContent = "Step away for five minutes."; focusGoalTitle.textContent = "Your next session can wait."; focusTime.innerHTML = `${formatTime(left)}<span>Remaining</span>`; focusStop.textContent = "Skip break"; overlay.classList.remove("hidden"); focusStop.onclick = () => { clearInterval(timerHandle); overlay.classList.add("hidden"); }; timerHandle = setInterval(() => { left -= 1; focusTime.innerHTML = `${formatTime(Math.max(left, 0))}<span>Remaining</span>`; if (left <= 0) { clearInterval(timerHandle); overlay.classList.add("hidden"); } }, 1000); }

async function loadHistory() { const sessions = await api("/sessions"); const history = document.getElementById("history-view"); history.innerHTML = sessions.length ? "" : '<p class="page-description">No focus sessions recorded yet.</p>'; sessions.forEach((session) => { const node = document.getElementById("session-template").content.cloneNode(true); node.querySelector(".session-title").textContent = session.task_title; node.querySelector(".session-meta").textContent = `${session.actual_minutes} of ${session.planned_minutes} minutes · ${new Date(session.created_at).toLocaleString()}`; const status = node.querySelector(".session-status"); status.textContent = session.completed ? "Completed" : "Ended early"; status.classList.add(session.completed ? "pill-success" : "pill-warning"); history.appendChild(node); }); }

function renderAgenda(goalsWithTasks) { const all = goalsWithTasks.flatMap(({ goal, tasks }) => leafTasks(tasks).map((task) => ({ task, goal }))).sort((a, b) => (a.task.completed - b.task.completed) || (a.task.priority - b.task.priority) || (a.task.id - b.task.id)); const list = document.getElementById("today-agenda"); list.innerHTML = ""; const display = all.slice(0, 4); display.forEach(({ task, goal }) => { const row = document.createElement("div"); row.className = `agenda-item${task.completed ? " done" : ""}`; row.innerHTML = `<button class="task-check" aria-label="Mark ${escapeHtml(task.title)} complete"></button><div><span class="agenda-title">${escapeHtml(task.title)}</span><span class="agenda-goal">${escapeHtml(goal.title)}</span></div><span class="agenda-priority">P${task.priority}</span><span class="agenda-time">${task.estimated_minutes}m</span>`; row.querySelector(".task-check").addEventListener("click", async () => { await api(`/tasks/${task.id}`, { method: "PATCH", body: JSON.stringify({ completed: !task.completed }) }); loadDashboard(); }); list.appendChild(row); }); if (!display.length) list.innerHTML = '<p class="page-description">Nothing queued. Add a step from Roadmap.</p>'; const done = all.filter((item) => item.task.completed).length; document.getElementById("daily-progress-copy").textContent = all.length ? `${all.length - done} remaining` : "No tasks planned"; document.getElementById("daily-progress-number").textContent = `${done} / ${all.length}`; document.getElementById("daily-progress-value").style.width = `${all.length ? (done / all.length) * 100 : 0}%`; }
function renderGoalsSummary(goalsWithTasks) { const container = document.getElementById("dashboard-goals"); container.innerHTML = ""; goalsWithTasks.slice(0, 4).forEach(({ goal, tasks }) => { const leaves = leafTasks(tasks), done = leaves.filter((task) => task.completed).length, percent = leaves.length ? (done / leaves.length) * 100 : 0; const row = document.createElement("div"); row.className = "goal-line"; row.innerHTML = `<div><div class="goal-name">${escapeHtml(goal.title)}</div><div class="goal-detail">${leaves.length ? `${leaves.length - done} steps remaining` : "No steps yet"}</div></div><div class="mini-progress"><div class="progress-track"><div class="progress-value" style="width:${percent}%"></div></div><small>${done}/${leaves.length}</small></div>`; container.appendChild(row); }); if (!goalsWithTasks.length) container.innerHTML = '<p class="page-description">No active goals.</p>'; }
...
async function renderMomentum(stats) { const sessions = await api("/sessions"); document.getElementById("stat-streak").textContent = stats.current_streak; document.getElementById("stat-sessions").textContent = stats.total_sessions; document.getElementById("stat-minutes").textContent = stats.total_minutes; document.getElementById("side-session-count").textContent = `${stats.total_sessions} ${stats.total_sessions === 1 ? "session" : "sessions"}`; renderActivityHeatmap(sessions); document.getElementById("rhythm-copy").textContent = stats.current_streak ? `${stats.current_streak}-day focus streak` : "Complete a session to start your activity record."; }
function renderWhatsNext(nextFocus) { const goalLabel = document.getElementById("whats-next-goal"), title = document.getElementById("whats-next-title"), action = document.getElementById("whats-next-action"), duration = document.getElementById("focus-duration"), time = document.getElementById("hero-time"); action.innerHTML = ""; if (nextFocus) { const { task, goal } = nextFocus; goalLabel.textContent = goal.title; title.textContent = task.title; duration.textContent = `${task.estimated_minutes} MIN`; time.innerHTML = `${String(task.estimated_minutes).padStart(2, "0")}:00<span>Focus</span>`; const begin = document.createElement("button"); begin.className = "primary-btn"; begin.textContent = "Start focus"; begin.addEventListener("click", () => startFocus(task)); const skip = document.createElement("button"); skip.className = "secondary-btn"; skip.textContent = "Roadmap"; skip.addEventListener("click", () => switchView("goals")); action.append(begin, skip); return; } goalLabel.textContent = "No task selected"; title.textContent = "Choose what to work on."; duration.textContent = "OPEN"; time.innerHTML = "—<span>Focus</span>"; const roadmap = document.createElement("button"); roadmap.className = "primary-btn"; roadmap.textContent = "Open roadmap"; roadmap.addEventListener("click", () => switchView("goals")); action.appendChild(roadmap); }
async function loadNextFocus() { try { return await api("/next-focus"); } catch (error) { if (error.status === 404) return null; throw error; } }
async function loadDashboard() { setDateCopy(); renderActivityHeatmap([]); const [goals, nextFocus] = await Promise.all([api("/goals"), loadNextFocus()]); const goalsWithTasks = await Promise.all(goals.map(async (goal) => ({ goal, tasks: await api(`/goals/${goal.id}/tasks`) }))); renderWhatsNext(nextFocus); renderAgenda(goalsWithTasks); renderGoalsSummary(goalsWithTasks); renderMomentum(await api("/stats")); }

const views = ["dashboard", "goals", "history"];
function switchView(name) { views.forEach((view) => { document.getElementById(`view-${view}`).classList.toggle("hidden", view !== name); document.getElementById(`nav-${view}`).classList.toggle("active", view === name); }); if (name === "dashboard") loadDashboard(); if (name === "goals") loadGoals(); if (name === "history") loadHistory(); window.scrollTo({ top: 0, behavior: matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" }); }
