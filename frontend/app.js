...
function startFocus(task) { clearInterval(timerHandle); const planned = task.estimated_minutes * 60; let left = planned; let logged = false; focusPhaseLabel.innerHTML = "<i></i>Focus block"; focusTaskTitle.textContent = task.title; focusGoalTitle.textContent = "One task. One protected interval."; focusTime.innerHTML = `${formatTime(left)}<span>Stay with it</span>`; focusStop.textContent = "End session"; overlay.classList.remove("hidden"); const log = (completed) => { if (logged) return; logged = true; return api(`/tasks/${task.id}/sessions`, { method: "POST", body: JSON.stringify({ actual_minutes: Math.floor((planned - left) / 60), completed }) }).then(loadDashboard); }; focusStop.onclick = () => { clearInterval(timerHandle); overlay.classList.add("hidden"); log(false); }; timerHandle = setInterval(() => { left -= 1; focusTime.innerHTML = `${formatTime(Math.max(left, 0))}<span>Stay with it</span>`; if (left <= 0) { clearInterval(timerHandle); log(true); startBreak(); } }, 1000); }
