...
function renderWhatsNext(nextFocus) { const goalLabel = document.getElementById("whats-next-goal"), title = document.getElementById("whats-next-title"), action = document.getElementById("whats-next-action"), duration = document.getElementById("focus-duration"), time = document.getElementById("hero-time"); action.innerHTML = ""; if (nextFocus) { const { task, goal } = nextFocus; goalLabel.textContent = goal.title; title.textContent = task.title; duration.textContent = `${task.estimated_minutes} MIN FOCUS`; time.innerHTML = `${String(task.estimated_minutes).padStart(2, "0")}:00<span>Focus block</span>`; const begin = document.createElement("button"); begin.className = "primary-btn"; begin.textContent = "Begin focus"; begin.addEventListener("click", () => startFocus(task)); const skip = document.createElement("button"); skip.className = "secondary-btn"; skip.textContent = "View roadmap"; skip.addEventListener("click", () => switchView("goals")); action.append(begin, skip); return; } goalLabel.textContent = "The next interval is open"; title.textContent = "Set a worthy direction for today."; duration.textContent = "OPEN DAY"; time.innerHTML = "—<span>Focus block</span>"; const roadmap = document.createElement("button"); roadmap.className = "primary-btn"; roadmap.textContent = "Set a direction"; roadmap.addEventListener("click", () => switchView("goals")); action.appendChild(roadmap); }
