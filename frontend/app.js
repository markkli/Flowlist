...
async function loadDashboard() { setDateCopy(); renderActivityHeatmap([]); const goals = await api("/goals"); const goalsWithTasks = await Promise.all(goals.map(async (goal) => ({ goal, tasks: await api(`/goals/${goal.id}/tasks`) }))); renderWhatsNext(goalsWithTasks); renderAgenda(goalsWithTasks); renderGoalsSummary(goalsWithTasks); renderMomentum(await api("/stats")); }
